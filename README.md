...
**Roster model overrides now reach every dispatch lane.** `invoke_codex` gained a `CODEX_MODEL` override (same pattern as `AGY_MODEL`/`OPENCODE_MODEL`/`KIMI_MODEL`/`CURSOR_MODEL`), and `dispatch_role`'s codex lane passes the resolved roster model through it — a customized reviewer/tester model actually reaches `codex exec -m` instead of silently deferring to the `agents.toml` pin. The shipped default matches the pin, so behavior is unchanged until a user customizes.
