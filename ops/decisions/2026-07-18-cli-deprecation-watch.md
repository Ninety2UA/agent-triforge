...
Capability claims rest on the U1 machine probe record `ops/research/2026-07-probe-record.md` (re-run 2026-07-18: 49 probes — 31 PASS / 15 FAIL / 1 AUTH-FAIL / 1 SKIPPED / 1 PENDING; the FAILs are the expected/handled set — agy sandbox/hooks/deny inert on 1.1.3, OpenCode/Kimi host-auth, Cursor headless-hooks/sandbox, monitors parity — each with a documented fallback). Per the watch-cycle method, a verdict that flips a prior ADR must cite a re-run probe row; the D-004 reversal does (CDX-04) and is recorded in full in its own ADR — this one references it rather than duplicating it.
