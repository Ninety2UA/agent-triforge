...
- Every dispatched builder receives the same contract (no sub-dispatch, git stays local — pushes are refused mechanically inside a lease worktree, SELF-09 — a typed `Status:` report); a bare exit 0 is never review-ready — a missing report is "report missing" (rc 80), not "no findings" (see "Dispatch contract + completion signal")
...
- Protected-path diffs (permission configs, deny rules, `ops/roster.toml` incl. `[promotion]`, shipped agent configs, and the framework's own control-plane code — `scripts/invoke-external.sh` and its `scripts/lib/*.sh` lanes, `scripts/lease-git-hooks/*`, `scripts/coordinate.sh`, `scripts/probe-capabilities.sh` and `scripts/probe-self-tests.sh`, hook handlers, `.claude/settings*.json`) force the promotion gate on and require the lead or user as the cross-reviewer — never an external-CLI-only review
...
scripts/
  ...
  lib/
    ...
    kimi.sh                 invoke_kimi (--agent-file)
    cursor.sh               _cursor_bin, _cursor_model_for_effort, invoke_cursor
    roster.sh               resolve_role, dispatch_role, roster_* (DEFAULTS / CLI_DEFAULT_MODEL live here — validate-versions.sh parses this file)
